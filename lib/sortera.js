		...
		var thead = table.querySelector("thead"),
			...
			sort = function (colIdx, specificOrder) {
				...
				if (sortOrder.length > 0 ) {
					...
					// Add sort-arrows
					for(i=0;i<sortOrder.length;i++) {
						// Which extra chacter to use if there is more than one sorted column?
						colExponentCharacter = sortOrder.length > 1 ? (i+1 > 3 ? "°" : "&sup"+(i+1)+";") : "";
						// Add sort arrow and character to current column
						ths[sortOrder[i].col].innerHTML += "<a class=\"s-arrow\"> " + (sortOrder[i].order == 1 ? "&#9660" : "&#9650;") + colExponentCharacter +"</a>";
					}
